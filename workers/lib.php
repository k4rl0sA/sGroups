...
function lis_employee() {
    ...
    $sql = "SELECT U.`id_usuario` AS ACCIONES,U.id AS 'Cod. Empleado', U.id_usuario AS Documento,nombre,CTLG(7,area) 'Area de la Empresa',CTLG(1,departamento) AS Departamento, 
  ...
  function cmp_employee(){
    ...
    function get_employee(){
		if($_POST['id']=='0'){
			...
			$id=divide($_POST['id']);
			$sql="SELECT id,id_usuario,nombre,area,departamento,ciudad,perfil,n_contacto telefono,eps,arl,correo,estado  FROM usuarios WHERE id_usuario='".$id[0]."'";
